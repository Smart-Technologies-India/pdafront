      ...
      if (!addpayment.status) {
        ...
        if (data.status) {
          ...
          const savepaymentdoc = await ApiCall({
            query: `
                        mutation updateRtiById($updateRtiInput:UpdateRtiInput!){
                            updateRtiById(updateRtiInput:$updateRtiInput){
                              id,
                            }
                          }
                        `,
...
const QueryTabs: React.FC<QueryTabsProps> = (
  ...
  return (
    <div
      ...
      } gap-4 items-center my-4`}
    >
      <div
        className={`w-10 h-10 shrink-0 rounded-full text-white font-semibold text-xl ${
          props.isUser ? "order-2 bg-green-500" : "order-1 bg-cyan-500"
        } grid place-items-center`}
      >
        {props.isUser
          ? props.from_user.toString().slice(0, 1).toUpperCase()
          : props.to_user.toString().slice(0, 1).toUpperCase()}
      </div>
      <div
        className={`px-4 py-2 rounded-md ${
          props.isUser ? "bg-[#e2eaf0] order-1" : "bg-[#dbf4fe] order-2"
        }`}
      >
        <div className="flex w-full border-b-2 border-gray-500 pb-2 mb-2">
          ...
          <p className={`text-sm text-gray-700 `}>
            {props.from_user} to {props.to_user}
          </p>
        </div>
